import { useNavigation, useRoute } from "@react-navigation/native";
import React from "react";
import { Text, TouchableOpacity, View } from "react-native";
import { MaterialIcons } from "@expo/vector-icons";
import { selectedRoutes } from "../../utils/operation/routes";
import { useAuth } from "../../../context/auth";

export function TabsHeader({ direction = "horizontal" }) {
  const navigation = useNavigation();
  const { userGust, user } = useAuth();

  const route = useRoute();
  const activeTab = route.name;
  let tabs = selectedRoutes(userGust).tabs;

  return (
    <View
      className={`flex ${direction === "horizontal" ? "flex-row gap-4" : "flex-col gap-6"}`}
    >
      {tabs.map((tab) => (
        <TouchableOpacity
          key={tab.name}
          onPress={() =>
            navigation.reset({
              index: 0,
              routes: [{ name: tab.routePath }],
            })
          }
          className={`flex items-center justify-center px-4 py-2 rounded-lg duration-300 transition-all ${
            activeTab === tab.routePath
              ? "bg-accent text-body"
              : "bg-transparent text-muted hover:bg-accent hover:text-body"
          }`}
        >
          <View
            className={`flex ${direction === "horizontal" ? "flex-row items-center" : "flex-col items-center"} gap-1`}
          >
            <MaterialIcons
              name={tab.icon}
              size={22}
              className={`${
                activeTab === tab.routePath
                  ? "text-body"
                  : "text-primary-custom"
              }`}
            />
            <Text
              className={`${
                activeTab === tab.routePath
                  ? "text-body"
                  : "text-primary-custom"
              } font-medium`}
            >
              {tab.name}
            </Text>
          </View>
        </TouchableOpacity>
      ))}
    </View>
  );
}
